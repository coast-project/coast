...
public:
    ...
    template <typename InputIterator> Anything(InputIterator first, InputIterator last) : fAnyImp(0) {
    ...
    template <typename InputIterator> Anything(InputIterator first, InputIterator last, Allocator *a) : fAnyImp(0) {
        SetAllocator(a);
        for (; first != last; ++first) {
            Append(*first);
        }

...
bool MmapStreamBuf::xinit(int fd) {
	...
	setg((fFl.IsReadable()) ? (char *)fAddr : (char *)fAddr + fFileLength - fMapOffset, (char *)fAddr + getoffset,
...
//
MmapStreamBuf::pos_type MmapStreamBuf::seekoff(MmapStreamBuf::off_type of, MmapStreamBuf::seekdir dir,
											   MmapStreamBuf::openmode mode) {
...
MmapStreambase::MmapStreambase(const char *name, int omode, int prot, int syncflag) : fMmapBuf(syncflag) {

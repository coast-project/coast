...
public:
	...
	int GetIOSMode() { return fOpenMode; }
	int GetSyscallOpenMode() { return fSyscallOpenMode; }
	...
	bool IsIosApp() { return TestIos(std::ios::app); }
	bool IsIosAppendOrAtEnd() { return TestIos(std::ios::app | std::ios::ate); }
	void IosClearAtEnd() {
...
private:
	...
	char *start() { return (char *)fAddr; }
	char *end() { return (char *)fAddr + fLength - fMapOffset; }
...
public:
	...
	MmapStreambase() { init(&fMmapBuf); }
	//! normal constructor opening a file corresponds to fstream ctor
	MmapStreambase(const char *name, int mode = std::ios::in, int prot = MmapStreamBuf::openprot,
	...
	//! let streambuf leak
	MmapStreamBuf *rdbuf() { return &fMmapBuf; }
...
public:
	...
	IMmapStream(const char *path) : MmapStreambase(path, std::ios::in), std::istream(&fMmapBuf) {}
	~IMmapStream() {}
};	// IMmapStream
...
public:
	...
	OMmapStream(const char *path, int mode = std::ios::out, int prot = MmapStreamBuf::openprot,
				int syncflag = MmapStreamBuf::eSync)
		: MmapStreambase(path, std::ios::out | mode, prot, syncflag), std::ostream(&fMmapBuf) {}
	virtual ~OMmapStream() {}
};	// OMmapStream
...
public:
	...
	MmapStream(const char *path, int mode = std::ios::out | std::ios::in, int prot = MmapStreamBuf::openprot,
			   int syncflag = MmapStreamBuf::eSync)
		: MmapStreambase(path, mode, prot, syncflag), std::iostream(&fMmapBuf) {}
	virtual ~MmapStream() {}
};	// MmapStream

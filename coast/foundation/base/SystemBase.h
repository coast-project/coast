...
namespace coast {
	namespace system {
		...
		String GenTimeStamp(char const *format = "%Y%m%d%H%M%S", bool const useLocalTime = true,
							time_t const referenceTime = time(0));
		...
		/*! \note: it is considered very unsafe for CGI to use the environment of the running server, better a sandbox
		  environment is used

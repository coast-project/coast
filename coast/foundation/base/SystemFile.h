...
namespace coast {
	namespace system {
		...
		/*! \deprecated use OpenOStream(const String&, openmode)
		  ...
		  \param log turn SystemLog::Debug messages on or off
		  \return the pointer to the open iostream or NULL, the client is responsible for destruction */
		std::iostream *OpenOStream(const char *name, const char *extension, openmode mode = std::ios::out | std::ios::trunc,
								   bool log = false);
		...
		/*! Since OpenStream uses fgRootDir and fgPathList we have to make sure that they contain reasonable defaults whenever
		  they are used. fgRootDir is initialized to the environment variable COAST_ROOT if set, to '.' otherwise fgPathList is
		  initialized to the environment variable COAST_PATH if set, to ".:config:src" if root is set and to
		  ".:../config:../src:" if root is not set
		...
		enum DirStatusCode {
			...
			eNoMoreHardlinks = (eFailed | (eSuccess << 7)),	 //! directory creation failed because no more hardlinks available
															 //! (symlinks are still possible?!)
			...
			eNoSpaceLeft = (eFailed | (eSuccess << 12)),  //! no more room - free inodes - to create directory, fails also, when
														  //! fragmentation of FS is bad! -> use fsck to examine
			eCreateSymlinkFailed = (eFailed | (eSuccess << 13)),  //! was not able to create symbolic link
			eCreateDirFailed =
				(eFailed | (eSuccess << 14)),  //! was not able to create directory for a reason not specified yet
			ePathIncomplete =
				(eFailed | (eSuccess << 15)),  //! given path was not complete or an operation on it lead to failures
		...
		//! create new directory with given permissions, works for relative or absolute path names and also recursive if
		//! specified.
		...
		/*! It is not intended to extend the system specific functionality to handle unsafe conditions or whatever but to
		  present an API which hides system specific differences or irregularities. */
		namespace io {

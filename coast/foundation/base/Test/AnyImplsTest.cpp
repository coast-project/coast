...
void AnyImplsTest::ThisToHexTest() {
	...
	{
		boost_or_std::auto_ptr<AnyImpl> d =
			boost_or_std::auto_ptr<AnyImpl>(new (coast::storage::Current()) AnyLongImpl(123L, coast::storage::Current()));
		String res = d->ThisToHex();
		assertCompare(sizeof(void *) * 2L, equal_to, static_cast<unsigned long>(res.Length()));
		StringStream os;
		os << std::hex << std::setw(sizeof(d.get()) * 2) << std::setfill('0') << std::noshowbase
		   << reinterpret_cast<unsigned long long>(static_cast<void *>(d.get()));

...
void StringTest::constructors() {
	...
	t_assert(memcmp((const char *)emptyCap, "", strlen((const char *)emptyCap)) == 0);
...
void StringTest::appendsTwoHexAsChar() {
	...
	// Test Methode AppendTwoHexAsChar(const char *)
	{
		...
		for (i = 0; i < 256; i++) {
			for (j = 0; j < 256; j++) {
				if (((char)i < '0') && ((char)i > '9') && ((char)i < 'a') && ((char)i > 'f') && ((char)i < 'A') &&
					((char)i > 'F') && ((char)j < '0') && ((char)j > '9') && ((char)j < 'a') && ((char)j > 'f') &&
					((char)j < 'A') && ((char)j > 'F')) {
					a[0] = (char)i;
					a[1] = (char)j;
				...
				*os1 << str;
				...
				if (is1 != 0) {
					*is1 >> str1;
					...
					t_assert(memcmp((const char *)str, (const char *)str1, str.Length()) <
							 0);  // str1 does not have the leading SPACE ????

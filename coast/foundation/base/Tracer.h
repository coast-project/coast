...
If the preprocessor flag \em COAST_TRACE is not set, the macros described here expand into nothing. To keep the trace output at
acceptable levels we introduced a config file for debugging, \b Tracer.any. Therein you can switch the trace on and off at a
global level but also at method level if needed.

\par Trace mechanics
Trace statements are scoped unless the Static ones are used. In every scope of a C++-Program you can use a StartTrace() or
StartTrace1() macro. They take as a first parameter a trigger e.g. <B>StartTrace(Server.Load)</B>. This results into output onto
cerr like
\code
...
Server.Load --- leaving ---
\endcode
on entering resp. leaving the scope. Within the scope you can place as many Trace() macros as you need. They are all enabled
with the same switch. To enable them, you have to insert the following lines into \b Tracer.any:
\code
{
  /LowerBound		10
  /UpperBound		20
...
public:
	...
	/*! \param trigger Will internally be used to do an Anything::LookupPath() search inside the \b Tracer.any file to check if
	 * trace output should be enabled or not */
...
#define StatTraceAny(trigger, any, msg, allocator)                           \
	{                                                                        \
		String gsMecart(allocator);                                          \
		gsMecart << msg; /* NOLINT(bugprone-macro-parentheses) */            \
		Tracer::AnythingWDDebug(_QUOTE_(trigger), any, gsMecart, allocator); \
	}

/*! helper to check if trigger is enabled
  \param trigger scope of trigger to check for */
#define TriggerEnabled(trigger) Tracer::CheckWDDebug(_QUOTE_(trigger))

...
Pipe::Pipe(int readfd, int writefd, bool doClose, long timeout)
	: fReadFd(readfd), fWriteFd(writefd), fStream(0), fDoClose(doClose), fTimeout(timeout), fHadTimeout(false) {
	StartTrace1(Pipe.Pipe, "ReadFd:" << static_cast<long>(readfd) << " WriteFd:" << static_cast<long>(writefd)
									 << (doClose ? "" : " non") << " closing");
}

Pipe::Pipe(long timeout) : fReadFd(-1), fWriteFd(-1), fStream(0), fDoClose(false), fTimeout(timeout), fHadTimeout(false) {
...
bool Pipe::IsReady(bool forreading, long timeout) {
	...
	if (res < 0) {
		String logMsg;
		SystemLog::Error(logMsg << "Pipe, select failed, error number: " << system::GetSystemError() << " <"
								<< SystemLog::LastSysError() << "> return code " << res);
	}
	return res > 0;
}

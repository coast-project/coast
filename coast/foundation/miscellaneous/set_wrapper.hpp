...
template <typename R = void, template <typename, class> class CatchAll = DefaultCatchAll, bool ConstVisitable = false>
class BaseVisitable;
...
*/
template <typename T, class MutexPolicy = DummyMutex, template <typename> class AllocType = DefaultAllocatorGlobalType,
...
public:
	...
	set_wrapper(const char *pName = "set_wrapper_lock") : fpList(NULL), fLock(pName, coast::storage::Global()) {}
	...
	bool FindItemRange(const ItemType &aItemMin, const ItemType &aItemMax, ConstIteratorType &rItemsBegin,
		...
		swStartTrace1(set_wrapper.FindItemRange,
					  "find items for range [" << aItemMin.AsString() << "," << aItemMax.AsString() << "]");
		bool bFound(false);
		if (HasList()) {
			LockUnlockEntry aGuard(*const_cast<MutexPolicy *>(&fLock));
	...
	bool ReduceToRange(const ItemType &aItemMin, const ItemType &aItemMax) {
		swStartTrace1(set_wrapper.ReduceToRange,
					  "reduce items to range [" << aItemMin.AsString() << "," << aItemMax.AsString() << "]");
	...
	bool DeleteItemRange(const IteratorType &rItemsBegin, const IteratorType &rItemsEnd) {
		swStartTrace1(set_wrapper.DeleteItemRange,
					  "[" << ((rItemsBegin != IntGetConstListPtr()->end()) ? (*rItemsBegin).AsString() : "<end>") << ","
						  << ((rItemsEnd != IntGetConstListPtr()->end()) ? (*rItemsEnd).AsString() : "<end>") << ")");
		bool bRet(false);
		if (HasList()) {
			LockUnlockEntry aGuard(fLock);
	...
	bool DeleteItemRange(const ItemType &aItemMin, const ItemType &aItemMax) {
		swStartTrace1(set_wrapper.DeleteItemRange,
					  "deleting items for range [" << aItemMin.AsString() << "," << aItemMax.AsString() << "]");
		bool bRet(false);
		if (HasList()) {
			LockUnlockEntry aGuard(fLock);
	...
	set_wrapper &operator=(const set_wrapper &other) { return Assign(other); }
...
protected:
	...
	bool IntFindItemRange(const ItemType &aItemMin, const ItemType &aItemMax, ConstIteratorType &rItemsBegin,
						  ConstIteratorType &rItemsEnd) const {
		swStartTrace1(set_wrapper.IntFindItemRange,
					  "find items for range [" << aItemMin.AsString() << "," << aItemMax.AsString() << "]");
		bool bFound(false);
		if (IntHasList()) {
			...
			swTrace("[" << ((rItemsBegin != aEnd) ? (*rItemsBegin).AsString() : "<end>") << ","
						<< ((rItemsEnd != aEnd) ? (*rItemsEnd).AsString() : "<end>") << ")");
	...
	bool IntReduceToRange(const ItemType &aItemMin, const ItemType &aItemMax) {
		swStartTrace1(set_wrapper.IntReduceToRange,
					  "reduce items to range [" << aItemMin.AsString() << "," << aItemMax.AsString() << "]");
		bool bModified(false);
		if (IntHasList()) {
			...
			if ((aBoundPos != aEnd) && (aBoundPos != IntGetListPtr()->begin())) {
				swTrace("removing [" << IntGetListPtr()->begin()->AsString() << ","
									 << ((aBoundPos != aEnd) ? aBoundPos->AsString() : "<end>") << ")");
	...
	bool IntAddItem(const ItemType &aItem) {
		...
		swStatTrace(set_wrapper.IntAddItem,
					"Item [" << aItem.AsString() << "] was " << (aRetCode.second ? "" : "not ") << "successful",

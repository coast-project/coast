...
#if defined(_RWSTD_VER) || defined(__SGI_STL_PORT) || BOOST_WORKAROUND(__BORLANDC__, BOOST_TESTED_AT(0x582))
...
namespace stlstorage {
	...
	public:
		...
		struct rebind {
			...
			rebind() { _StartTrace(STLAllocator.rebind); }
			~rebind() { _StartTrace(STLAllocator.~rebind); }
		};
		...
		~STLAllocator() { _StartTrace(STLAllocator.~STLAllocator); }
		...
		void deallocate(pointer p, size_type num) { fAllocator->Free((void *)p); }
	...
	public:
		...
		explicit pool_refcounted(const sz_type nrequested_size, const sz_type nnext_size)
			: fRefcount(0), fReqSz(nrequested_size), fNextSz(nnext_size), fpPool(0), fOtherPools() {
			_StatTrace(pool_refcounted.pool_refcounted, "sizeof:" << sizeof(int_pool_type) << " @" << static_cast<long>(this),
		...
		~pool_refcounted() {
			_StartTrace1(pool_refcounted.~pool_refcounted,
						 "refcnt:" << fRefcount << " fpPool:" << static_cast<long>(fpPool) << " @" << static_cast<long>(this));
		...
		template <class P>
		P Clone() {
			_StartTrace1(pool_refcounted.Clone,
						 "Clone<P> for size:" << static_cast<long>(fReqSz) << " @" << static_cast<long>(this));
			AddRef();
			return this;
		}
		...
		pool_refcount_storer Clone(sz_type nrequested_size, sz_type nnext_size) {
			_StartTrace1(pool_refcounted.Clone,
						 "Clone for size:" << static_cast<long>(nrequested_size) << " @" << static_cast<long>(this));
		...
		sz_type PoolSize() const { return fReqSz; }
		sz_type NextSize() const { return fNextSz; }

		void AddRef() {
			++fRefcount;
			_StatTrace(pool_refcounted.AddRef, "new refcnt:" << fRefcount << " @" << static_cast<long>(this),
					   coast::storage::Current());
		}

		bool Release() {
			--fRefcount;
			_StatTrace(pool_refcounted.Release, "new refcnt:" << fRefcount << " @" << static_cast<long>(this),
					   coast::storage::Current());
			return (fRefcount <= 0);
		}

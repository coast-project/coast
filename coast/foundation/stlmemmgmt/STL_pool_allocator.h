...
namespace stlstorage {
	...
	public:
		...
		pool_allocator(const ThisType &aAllocator) : fpIntPool(aAllocator.fpIntPool) {
			_StartTrace1(pool_allocator.pool_allocator,
						 "copy this:" << static_cast<long>(this) << " sizeof(T):" << sizeof(T) << " other:" << (long)&aAllocator
									  << " sizeof(other):" << sizeof(typename ThisType::value_type));
			_Trace("IntPool @" << (long)fpIntPool.get());
		}

		template <typename U>
		pool_allocator(const pool_allocator<U, UserAllocator, NextSize> &aAllocator) : fpIntPool() {
			_StartTrace1(pool_allocator.pool_allocator,
						 "copy other type this:" << static_cast<long>(this) << " sizeof(T):" << sizeof(T)
												 << " other:" << (long)&aAllocator << " sizeof(U):"
												 << sizeof(typename pool_allocator<U, UserAllocator, NextSize>::value_type));
		...
		~pool_allocator() {
			_StatTrace(pool_allocator.~pool_allocator,
		...
		pointer allocate(const size_type n) COAST_NOEXCEPT_IF(std::bad_alloc) {
			...
			_StatTrace(pool_allocator.allocate_n,
					   "this:" << static_cast<long>(this) << " ptr:" << static_cast<long>(ptr)
							   << " size:" << static_cast<long>(n) << '*' << sizeof(T) << " calling ordered_malloc "
							   << " fpIntPool:" << (long)fpIntPool.operator->(),
		...
		pointer allocate(const size_type n, const void *const) COAST_NOEXCEPT_IF(std::bad_alloc) {
			_StatTrace(pool_allocator.allocate_n_void,
					   "this:" << static_cast<long>(this) << " size:" << static_cast<long>(n) << '*' << sizeof(T) << " void*",
					   coast::storage::Current());
			return allocate(n);
		}
		void deallocate(const pointer ptr, const size_type n) {
			_StatTrace(pool_allocator.deallocate_ptr_n,
					   "this:" << static_cast<long>(this) << " ptr:" << static_cast<long>(ptr)
							   << " size:" << static_cast<long>(n) << '*' << sizeof(T) << " calling ordered_free "
							   << " fpIntPool:" << (long)fpIntPool.operator->(),
					   coast::storage::Current());
	...
	template <typename T>
	struct pool_allocator_global : public stlstorage::pool_allocator<T, itostorage::BoostPoolUserAllocatorGlobal, 32> {};
	template <typename T>
	struct pool_allocator_current : public stlstorage::pool_allocator<T, itostorage::BoostPoolUserAllocatorCurrent, 32> {};
}  // namespace stlstorage

...
#endif
DiffTimer::DiffTimer(DiffTimer::eResolution resolution) : fStart(0), fResolution(resolution) {
...
DiffTimer::DiffTimer(const DiffTimer &dt) : fStart(dt.fStart), fResolution(dt.fResolution) {
	StartTrace(DiffTimer.DiffTimer);
	Trace("TicksPerSecond(): " << TicksPerSecond() << " fResolution: " << fResolution);
	if (fResolution <= 0) {
		fResolution = eClockTicks;	// measure in clock ticks
...
DiffTimer::tTimeType DiffTimer::Scale(tTimeType rawDiff, DiffTimer::eResolution resolution) {
	...
	if (resolution > 0) {
		if (TicksPerSecond() < resolution) {
			...
			// beware of overflow
			scaled = static_cast<DiffTimer::tTimeType>(
				static_cast<double>(rawDiff) * (static_cast<double>(resolution) / static_cast<double>(TicksPerSecond())));
		}
	}
	StatTrace(DiffTimer.Scale,
			  "TicksPerSecond(): " << TicksPerSecond() << " resolution: " << resolution << " rawDiff: " << rawDiff
								   << " scaled: " << scaled,

...
public:
	...
	static TimeStamp Min() { return TimeStamp(19, 70, 1, 1, 0, 0, 0); }
	static TimeStamp Max() { return TimeStamp(20, 37, 12, 31, 23, 59, 59); }

	enum eWeekday { eSunday = 0, eMonday = 1, eTuesday = 2, eWednesday = 3, eThursday = 4, eFriday = 5, eSaturday = 6 };

	char Century() const { return fTimeStruct.Century(); }
	long Year() const { return fTimeStruct.Year(); }
	char YearShort() const { return fTimeStruct.YearShort(); }
	char Month() const { return fTimeStruct.Month(); }
	const char *MonthName() const;
	char Day() const { return fTimeStruct.Day(); }
	const char *DayName() const;
	char Hour() const { return fTimeStruct.Hour(); }
	char Minute() const { return fTimeStruct.Minute(); }
	char Second() const { return fTimeStruct.Second(); }
	...
	inline eWeekday Weekday() const { return fTimeStruct.Weekday(); }
	/*! return the day number beginning at 1 for the first of january
	  \return day number beginning at 1 */
	inline int DayOfYear() const { return fTimeStruct.DayOfYear(); }
	...
	inline int WeekOfYear() const { return fTimeStruct.WeekOfYear(); }
	...
	bool operator>(const TimeStamp &isAfter) const { return IsAfter(isAfter); };
	bool operator<(const TimeStamp &isBefore) const { return IsBefore(isBefore); };
	bool operator==(const TimeStamp &isEqual) const { return IsEqual(isEqual); };
	bool operator!=(const TimeStamp &isNotEqual) const { return IsNotEqual(isNotEqual); };
	bool operator>=(const TimeStamp &isAfterEqual) const { return IsAfterEqual(isAfterEqual); };
	bool operator<=(const TimeStamp &isBeforeEqual) const { return IsBeforeEqual(isBeforeEqual); };
	...
	static inline bool IsLeap(int iYear) { return ((iYear) % 4 == 0 && ((iYear) % 100 != 0 || (iYear) % 400 == 0)); }
	...
	public:
		...
		explicit AMonth(eMonthNumber aMonth) : fMonth(aMonth) {}
	...
	public:
		explicit AYear(long lYear) : fYear(lYear) {}
...
protected:
	struct intTimeRep {
		enum eItemPositions { eCent = 0, eYear = 1, eMonth = 2, eDay = 3, eHour = 4, eMin = 5, eSec = 6, eMax = 7 };
		char cData[eMax];
		...
		char Century() const { return cData[eCent]; }
		long Year() const { return (cData[eCent] * 100) + cData[eYear]; }
		char YearShort() const { return cData[eYear]; }
		char Month() const { return cData[eMonth]; }
		char Day() const { return cData[eDay]; }
		char Hour() const { return cData[eHour]; }
		char Minute() const { return cData[eMin]; }
		char Second() const { return cData[eSec]; }
		bool SetTime(unsigned char iHour, unsigned char iMin, unsigned char iSec);
	};

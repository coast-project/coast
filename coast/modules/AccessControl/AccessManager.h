...
public:
	...
	/*!	\param ctx Context to use for DataAccess etc.
	  \param uid user id to check
	  \return true if user id is accepted */
	virtual bool Validate(Context &ctx, String uid) { return Validate(uid); }
	...
	//! reset password of the given user (if supported) to a value defined by the behavior of the utilized
	//! UserDataAccessManager, context version
	/*!	\param ctx Context to use for DataAccess etc.
	  \param uid user id to reset password for
	  \return true, if the reset was successful */
	virtual bool ResetPassword(Context &ctx, String uid) { return ResetPassword(uid); }
	...
	/*! \param ctx Context to use for DataAccess etc.
	  ...
	  \param entity to be allowed: can be a role, a service or a permissible entity or something similar
	  \return true if the entity is allowed for the given 'who' */
	virtual bool IsAllowed(Context &ctx, String who, String entity) { return IsAllowed(who, entity); }

	//! return all ressources (permissible entitites, services, ...) that a specific user, role, group or set of groups is
	//! allowed to use
	...
	//! return all ressources (permissible entitites, services, ...) that a specific user, role, group or set of groups is
	//! allowed to use, context version
	/*! \param ctx Context to use for DataAccess etc.
	  \param who uid, role, group name or set of group names to get allowed entities for
	  \param allowed allowed entities for the given who
	  \return false if either the given 'who' is not valid or if no entities were found, true otherwise */

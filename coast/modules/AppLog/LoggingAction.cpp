...
bool LoggingAction::DoExecAction(String &transitionToken, Context &ctx, const ROAnything &config) {
	...
	if (channel.Length()) {
		...
		if (config.LookupPath(roaFormat, "Format")) {
			bRet =
				AppLogModule::Log(ctx, channel, roaFormat,
								  (AppLogModule::eLogLevel)config["Severity"].AsLong(static_cast<long>(AppLogModule::eINFO)));
		} else {
			bRet = AppLogModule::Log(
				ctx, channel, (AppLogModule::eLogLevel)config["Severity"].AsLong(static_cast<long>(AppLogModule::eINFO)));
		}
	}
	return bRet;
}

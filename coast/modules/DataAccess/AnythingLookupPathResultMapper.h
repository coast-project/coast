...
//! Uses Anything::LookupPath() on entries specified in mapper configuration and calls ResultMapper::DoPutAny() using the
//! retrieved value.
/*! Lookup values in the given Anything and map them again using the given configuration. Provide a MapperScript which extracts
and stores either parts of the converted Anything or the whole Anything.
\par Configuration
\code
{
  /key {
...
Example output of pseudo-configuration from above assuming we got the following stream to convert <b>{ /Slot { /To { /Lookup {
123 /a bcd } } /Something 123 } }</b>
...
Note that the slot \b ResultsInNoOutput does not exist because the LookupPath of \b NonExistingSlot resulted in a Null-Anything
and therefore no output slot will be created!
 */
class AnythingLookupPathResultMapper : public ResultMapper {
public:
	/*! @copydoc RegisterableObject::RegisterableObject(const char *) */
	AnythingLookupPathResultMapper(const char *name) : ResultMapper(name) {}

	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) AnythingLookupPathResultMapper(fName); }

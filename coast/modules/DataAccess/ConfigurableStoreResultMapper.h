...
/*! Sometimes it would be nice to store/map values directly into a store different from TmpStore, like the Session or Role
store.
 * This is where this ResultMapper can be used as it just retrieves the target Anything within the store given in the mapper
configuration.
...
public:
	...
	ConfigurableStoreResultMapper(const char *name) : ResultMapper(name) {}
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) ConfigurableStoreResultMapper(fName); }

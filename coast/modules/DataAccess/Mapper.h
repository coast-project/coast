...
public:
	MappersModule(const char *name) : WDModule(name) {}
...
/*! This Mapper supports behavior configuration. Put specific configuration into InputMapperMeta.any for the corresponding
Mapper alias name.
\par Configuration
\code
{
...
public:
	...
	//! Pulls out an Anything according to key. Clients use this method to pull out an Anything value with name key from the
	//! Mapper
	/*! @param key the name defines kind of value to get
	  @param value the value to be mapped
	  @param ctx the thread context of the invocation
	...
	//! Write to the ostream os values according to key. Clients use this method to write to the stream values associated with
	//! the key
	...
	//! Get mapper script for the given key if any. Subclasses can overwrite DoSelectScript() to avoid scripting (recursion) or
	 ...
	 * @param script script in which to check for the given key, if omitted or the key was not found, we will check using
	 * hierarchic Lookup() mechanism.
	 ...
	 */
	ROAnything SelectScript(const char *key, ROAnything script, Context &ctx) const { return DoSelectScript(key, script, ctx); }

	//! Defines the name space where to get values from, like a prefix. Subclasses can overwrite DoGetSourceSlot().
	/*! Default is to Lookup("SourceSlot") in our own config. If it is not found, \em fName.SourceSlot will be searched in
	 * Context. If it is still not found, an empty default value will be returned, i.e. the key will then be looked up directly
	 * without any prefix. The lookup will be of the form [sourceslot.]key, where [sourceslot] may itself be an
	 * Anything::LookupPath() like path.
	 ...
	 */
	String GetSourceSlot(Context &ctx) const { return DoGetSourceSlot(ctx); }
	 ...
	 */
	const char getDelim() const { return Lookup("Delim", ".")[0L]; }
	 ...
	 */
	const char getIndexDelim() const { return Lookup("IndexDelim", ":")[0L]; }
...
protected:
	...
	//! Almost the same as DoGetAny but it uses the given slotname as new key for further processing. This allows key
	//! redefinition based on slotnames which are not ParameterMappers.
	...
	//! Almost the same as DoGetStream but it uses the given slotname as new key for further processing. This allows key
	//! redefinition based on slotnames which are not ParameterMappers.
	/*! @param key the key usually defines the associated kind of input-value
	  @param os The stream to map values onto
	  @param ctx the context of the invocation
	  ...
	  @param slotname new key to use for further processing
	  @return returns true if the mapping was successful otherwise false */
	virtual bool DoGetStreamWithSlotname(const char *key, std::ostream &os, Context &ctx, ROAnything script,
										 const char *slotname);
	...
	virtual bool doGetValueWithSlotname(const char *key, Anything &value, Context &ctx, ROAnything script,
										const char *slotname) {
	...
	virtual bool doGetValueWithSlotname(const char *key, std::ostream &os, Context &ctx, ROAnything script,
										const char *slotname) {
	...
	/*! @param key the key usually defines the associated kind of input-value
	  ...
	  @param slotname new key to use for further processing
	  @return returns true if the mapping was successful otherwise false */
	virtual bool interpretMapperScriptEntry(const char *key, Anything &value, Context &ctx, ROAnything script,
	...
	/*! @param key the key usually defines the associated kind of input-value
	  ...
	  @param slotname new key to use for further processing
	  @return returns true if the mapping was successful otherwise false */
	virtual bool interpretMapperScriptEntry(const char *key, std::ostream &os, Context &ctx, ROAnything script,
											String const &slotname);

	//! Generate the config file name (without extension, which is assumed to be any). Is simply the concatenation of category
	//! and "Meta". If category is "ParameterMapper" we use "InputMapper" instead, to keep compatibility.
	virtual bool DoGetConfigName(const char *category, const char *, String &configFileName) const;

	//! Load an anything and make it available by storing a reference in fConfig. The default implementation uses the cache
	//! handler. If you provide your own implementation be careful about the lifetime of the loaded anything otherwise fConfig
...
public:
	...
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) EagerParameterMapper(fName); }
...
/*! This Mapper supports behavior configuration. Put the following into OutputMapperMeta.any for the corresponding Mapper alias
name.
 ...
 * If the FindResultMapper() call fails, the slotname will be used as new key to use for further put calls. This allows on the
fly renaming of put keys.
 *
*/
...
public:
	...
	//! Takes an float for mapping according to key. Clients use this method to push an float value with name key into the
	//! Mapper
	/*! @param key the name defines kind of input for the value
	  @param value the value to be mapped
	  @param ctx the thread context of the invocation
	...
	//! Takes an double for mapping according to key. Clients use this method to push an double value with name key into the
	//! Mapper
	/*! @param key the name defines kind of input for the value
	  @param value the value to be mapped
	  @param ctx the thread context of the invocation
	...
	//! Takes an String for mapping according to key. Clients use this method to push an String value with name key into the
	//! Mapper
	/*! @param key the name defines kind of input for the value
	  @param value the value to be mapped
	  @param ctx the thread context of the invocation
	...
	//! Takes an const Anything & for mapping according to key. Clients use this method to push an const Anything & value with
	//! name key into the Mapper
	/*! @param key the name defines kind of input for the value
	  @param value the value to be mapped
	  @param ctx the thread context of the invocation
	  @return returns true if the mapping was successful otherwise false */
	...
	//! Reads from istream is values according to key. Clients use this method to let the mapper read values associated with the
	//! key directly from a istream
	/*! @param key the name defines kind of input for the value
	...
	/*! Calls DoGetDestinationSlot to get the name - or empty string - which will be used as base path in tmpstore. The 'base'
	  path - the one returned here - will be stored in ResultMapper.DestinationSlot for later usage.
...
protected:
	...
	//! Almost the same as DoPutAny but it uses the given slotname as new key for further processing. This allows key
	//! redefinition based on slotnames which are not ResultMappers.
	/*! @param key the key usually defines the associated kind of output-value
	  @param value the value to be mapped
	  @param ctx the context of the invocation
	  @param script current mapper configuration as ROAnything
	...
	//! Reads from istream and puts its content according to key. Default behavior is to read in the content of the stream into
	//! an anything an call the DoPutAny() method. Clients use this method to let the mapper read values associated with the key
	//! directly from a istream.
	/*! @param key the key usually defines the associated kind of output-value
	...
	//! Almost the same as DoPutStream but it uses the given slotname as new key for further processing. This allows key
	//! redefinition based on slotnames which are not ResultMappers.
	/*! @param key the key usually defines the associated kind of output-value
	  @param is stream whose content will be mapped
	  @param ctx the context of the invocation
	...
	/*! Hook for breaking recursion in mapper script interpretation. Store the value in tmpstore under tmp.slot.key, where slot
	  is retrieved with GetDestinationSlot(). if slot == "", then value is stored under tmp.key directly key may NOT be empty
	  (fails otherwise).
	...
	//! Hook for breaking recursion in mapper script interpretation. Stores everything readable from the stream as a String
	//! using DoFinalPutAny.
	/*! @param key the key usually defines the associated kind of output-value
	  @param is stream whose content will be mapped
	  @param ctx the context of the invocation
	...
	//! Defines the base name space where to put values into tmpstore, default is "Mapper". May return a "."-separated path,
	//! such as x.y.z. If empty string, tmpstore will be used as base.
	/*!	@param ctx the context in which to look for the destination slot
	...
	/*!	@param key the key usually defines the associated kind of output-value
	  @param targetAny Anything reference into TmpStore to finally put values at. It uses DestinationSlot and key to get the
	  correct location in Context.
	  @param ctx the context of the invocation */
	virtual void DoGetDestinationAny(const char *key, Anything &targetAny, Context &ctx);

	//! Generate the config file name (without extension, which is assumed to be any) out of category and objName. The default
	//! implementation just takes the objName.
...
public:
	...
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) EagerResultMapper(fName); }
...
public:
	...
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) RootMapper(fName); }

protected:
	String DoGetDestinationSlot(Context &ctx, const char *pcDefault) { return ""; }
};
...
Scripting can be re-activated with /MapperScript { ... a script ... }, the script is then evaluated and the results are inserted
in place of the call.<br> Example:<pre>
/ConfigMapper {
  /foo "bar"
  /nested {
	/inner {
...
</pre>Result: exactly the config above, with the difference that the value of "calculated" slot will be assigned the results of
the call to MyMapper
*/
...
public:
	...
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) ConfigMapper(fName); }

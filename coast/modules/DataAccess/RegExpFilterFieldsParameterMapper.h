...
//! Calls ParameterMapper::DoGetAny() on named value entries matched by regular expressions. Positive and negative filtering can
//! be applied.
 ...
 * Optional\n
 * Positive regular expression rule filter, ParameterMapper::DoGetAny() will be called with the contents of the matched slot of
value.
 *
...
class RegExpFilterFieldsParameterMapper : public ParameterMapper {
	...
	bool getValueWithSlotname(const char *key, String const &strSlotname, Context &ctx, ValueType &value, ROAnything script);
	template <typename ValueType>
	bool getValueWithSlotnameCommon(const char *key, String const &slotname, Context &ctx, ValueType &value, ROAnything script,
									Anything &availableContent);
	template <typename ValueType>
	bool intInterpretMapperScript(const char *key, ValueType &value, Context &ctx, ROAnything script);

public:
	/*! @copydoc RegisterableObject::RegisterableObject(const char *) */
	RegExpFilterFieldsParameterMapper(const char *name) : ParameterMapper(name) {}

	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) RegExpFilterFieldsParameterMapper(fName); }

...
namespace {
	...
	void storeKey(MapperType &mapper, Context &ctx, String const &strKey) {
		...
		if (mapper.Lookup(_StoreKeyAt, roaStoreSpec)) {
			...
			StorePutter::Operate(anyToPut, ctx, mapper.Lookup(_Store, _TmpStore), Renderer::RenderToString(ctx, roaStoreSpec),
								 false);
		}
	}
	template <typename MapperType>
	ROAnything mySelectScript(MapperType &mapper, const char *key, ROAnything script, Context &ctx) {
		...
		if (script.IsNull() || not(script.LookupPath(roaReturn, key, mapper.getDelim(), mapper.getIndexDelim()) ||
								   script.LookupPath(roaReturn, _AllSelector, _lookupPathBlocker, _lookupPathBlocker))) {
			Trace("key not found in given script or Null-script, use hierarch-lookup mechanism now");
			mapper.Lookup(key, roaReturn, mapper.getDelim(), mapper.getIndexDelim()) ||
				mapper.Lookup(_AllSelector, roaReturn, _lookupPathBlocker, _lookupPathBlocker);
		}
...
bool RenderedKeyResultMapper::DoPutAnyWithSlotname(const char *key, Anything &value, Context &ctx, ROAnything roaScript,
												   const char *slotname) {
	StartTrace1(RenderedKeyResultMapper.DoPutAnyWithSlotname,
...
bool RenderedKeyResultMapper::DoPutStreamWithSlotname(const char *key, std::istream &is, Context &ctx, ROAnything roaScript,
													  const char *slotname) {
	StartTrace1(RenderedKeyResultMapper.DoPutStreamWithSlotname,
				"key [" << NotNull(key) << "] slotname [" << NotNull(slotname) << "]");
	Trace("Using slotname [" << slotname << "] as new key (not a mapper)");

...
<PRE>
{
  ...
  /OnChangeScript							optional, called every time the listbox gets a Changed event
  ...
  /SelectBox {				optional
	...
	/Options {								options for the selectbox
  ...
  /Navigation {				optional		if defined, enable navigation, default off
	...
	/SaveButton {			extends specification above with the following slot
	  ...
	  /PreDoScript			Rendererspec	optional, specify an inline script which extends
  ...
  /MyColList {
	{
	  ...
	  /BgColor			optional		background color of the cell. It's advisable to specify it only if MultifunctionListBoxRenderer.EditLine.BgColor isn't set,
			  	  	  	  	  	  	  	otherwise we'll get <td bgcolor=...> repeated for each cell, when it could be simply once defined in
										MultifunctionListBoxRenderer.EditLine.BgColor
	}
	{
	  ...
	}
...
private:
	...
	bool DoRenderButton(std::ostream &reply, Context &c, const ROAnything &navConfig, const ROAnything &config,
						const String &strButtonName);
...
protected:
	...
	void RenderSortIcon(std::ostream &reply, Context &c, const ROAnything &config, String strSortString, String strSortOrder,
						const ROAnything &imageConfig);

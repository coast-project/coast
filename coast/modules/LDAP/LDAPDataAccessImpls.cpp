...
int LDAPModifyDAI::IntPrepareLDAPMods(LDAPMod **ldapmods, int modcode, int &lModsCounter, ROAnything attrmods,
									  bool bBinaryOperation) {
	...
	for (long sza = attrmods.GetSize(); lCurrentAttribute < sza; ++lCurrentAttribute, ++lModsCounter) {
		...
		SubTrace(TraceMallocFree,
				 "preparing attribute [" << attrmods.SlotName(lCurrentAttribute) << "] (" << (long)lCurrentAttribute << ")");
		...
		for (i = 0; i < totalmods; ++i) {
			if (bBinaryOperation) {
				...
				while (ldapmods[i]->mod_bvalues != NULL && ldapmods[i]->mod_bvalues[mod_bvaluesIndex] != NULL) {
					SubTrace(TraceMallocFree,
							 "freeing ldapmods[" << (long)i << "]->mod_bvalues[" << (long)mod_bvaluesIndex << "]");

...
void PersistentLDAPConnection::tsd_setup() {
	...
	SystemLog::Info(String("PersistentLDAPConnection::tsdsetup() Setting thread specific data. Thread [")
					<< Thread::MyId() << "]");
}
...
LDAPConnection::EConnectState PersistentLDAPConnection::DoConnect(ROAnything bindParams, LDAPErrorHandler &eh) {
	...
	if (!SetProtocol(eh) || !SetConnectionTimeout(eh) || !SetSearchTimeout(eh) || !SetErrnoHandler(eh)
		/* || !SetRebindProc(eh) */) {
...
String PersistentLDAPConnection::GetLdapPoolId(ROAnything bindParams) {
	...
	return String("Host[") << fServer << "] Port[" << fPort << "] DN[" << bindName << "] BindPW[" << bindPW << "] ConnTimeout["
						   << fConnectionTimeout << "]";
}

String PersistentLDAPConnection::GetLdapPoolId(const String &server, long port, const String &bindName, const String &bindPW,
											   long connectionTimeout) {
	StartTrace(PersistentLDAPConnection.GetLdapPoolId);
	String armouredPW = Base64ArmouredMD5Hash(bindPW);

	return String("Host[") << server << "] Port[" << port << "] DN[" << bindName << "] BindPW[" << armouredPW

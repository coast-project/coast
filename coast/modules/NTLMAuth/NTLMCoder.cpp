...
namespace NTLMCoder {
	long decodeshort(const String &dm, long pos) { return (unsigned char)dm[pos] + ((unsigned char)dm[pos + 1L] << 8); }
	void appendshort(String &buff, unsigned short val) { buff.Append((char)(val & 0xff)).Append((char)(val >> 8)); }
	...
	bool DecodeServerNonce(const String &base64msg, String &nonce) {
		...
		String dm;
		if (DecodeBase64(dm, base64msg) && 40L == dm.Length() && "NTLMSSP" == dm.SubString(0L, 7L) && '\x02' == dm[8L] &&
			'\x28' == dm[16L] && '\0' == dm[17L] && '\x01' == dm[20L] && '\x82' == dm[21L]) {
	...
	bool DecodeResponse(const String &base64msg, const String &nonce, const String &lmhash, const String &nthash,
		...
		String dm;
		if (DecodeBase64(dm, base64msg) && "NTLMSSP" == dm.SubString(0L, 7L) && '\x03' == dm[8L] && '\x01' == dm[60L] &&
			'\x82' == dm[61L] && dm.Length() == decodeshort(dm, 56L)) {
			Trace("decoded string len: " << dm.Length());

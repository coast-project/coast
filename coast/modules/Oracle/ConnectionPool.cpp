	...
	namespace oracle {
		namespace {
			...
			public:
				...
				THREADKEY getConnectionKey() const { return fConnectionKey; }
				bool TLSUsable() const { return fTLSUsable; }
				WPMStatHandler *getStatHandlerTLS() const { return fpStatEvtHandlerTLS.get(); }
			};
		...
		protected:
			...
			virtual bool DoCleanup() {
				...
				if (GETTLSDATA(OracleTlsKeyInitializerSingleton::instance().getConnectionKey(), pConnection,
		...
		ConnectionPool::ConnectionPool(const char *name)
			: fStructureMutex(String(name).Append("StructureMutex"), coast::storage::Global()),
			  fListOfConnections(coast::storage::Global()),
			  fInitialized(false),
		...
		Allocator *getListAllocator(const ROAnything roaConfig) {
			...
			if (roaConfig["UsePoolStorage"].AsLong(1) == 1) {
				...
				if (pAlloc == NULL) {
					StatTrace(ConnectionPool.getListAllocator, "failed to create PoolAllocator, using coast::storage::Global()",
			...
			if (!fInitialized) {
				...
				LockUnlockEntry me(fStructureMutex);
				{
					...
					for (long i = 0; i < nrOfConnections; ++i) {
						OraclePooledConnection *pConnection = new (coast::storage::Global())
							OraclePooledConnection(i, myCfg["MemPoolSize"].AsLong(2048L), myCfg["MemPoolBuckets"].AsLong(16L));
		...
		bool ConnectionPool::BorrowConnection(OraclePooledConnection *&pConnection, const String &server, const String &user,
			...
			if (OracleTlsKeyInitializerSingleton::instance().TLSUsable() && bUseTLS) {
				if ((bRet = GETTLSDATA(OracleTlsKeyInitializerSingleton::instance().getConnectionKey(), pConnection,
									   OraclePooledConnection))) {
					// verify if current connection is for same server/user
					Trace("got connection from TLS, server [" << pConnection->getServer() << "], user ["
															  << pConnection->getUser() << "]");
					...
					Trace("need to create new Connection") Thread::RegisterCleaner(&fgCleaner);
				...
				while (!bDone && aTSIter.Next(roaTSEntry)) {
					...
					while (!bDone && aConnIter.Next(roaConn)) {
						if ((strToLookup.Length() <= 0) || strToLookup.IsEqual(roaConn[1L].AsCharPtr(""))) {
							...
							// we do not have to close the connection before using because the OraclePooledConnection is for the

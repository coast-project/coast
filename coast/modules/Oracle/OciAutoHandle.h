...
struct oci_auto_handle {
	 ...
	 */
	oci_auto_handle(handle_ptr_type pHandle) : fHandle(pHandle) {}
	//! frees allocated handle if stored internally
	~oci_auto_handle() { doDelete(); }
	 ...
	 * @return internal handle
	 * */
	handle_ptr_type getHandle() const { return fHandle; }
	ub4 getHandleType() const { return handleType; }
	/*! Accessor to use when creating a new handle.
	 * This method will give access to the handle data field which is needed to allocate a new handle of a given type in this
	 * object.
	 * @return address of the internal handle data area
	 * @note Calling this method repeatedly will delete the previously allocated handle object! Usually you just need to use
	 * this function once when initially allocating the resource handle.
	 */
	 ...
	 */
	operator const bool() const { return fHandle != 0; }

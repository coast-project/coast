...
bool OracleConnection::Open(String const &strServer, String const &strUsername, String const &strPassword) {
	...
	// --- attach server
	if (checkError(
			OCIServerAttach(fSrvhp.getHandle(), fErrhp.getHandle(), server, strlen((const char *)server), (ub4)OCI_DEFAULT),
			strErr)) {
	...
	if (checkError(OCIAttrSet(fSvchp.getHandle(), (ub4)OCI_HTYPE_SVCCTX, fSrvhp.getHandle(), (ub4)0, (ub4)OCI_ATTR_SERVER,
							  fErrhp.getHandle()),
				   strErr)) {
	...
	if (checkError(OCIAttrSet(fUsrhp.getHandle(), (ub4)OCI_HTYPE_SESSION, (dvoid *)username,
							  (ub4)strlen((const char *)username), (ub4)OCI_ATTR_USERNAME, fErrhp.getHandle()),
				   strErr)) {
		SystemLog::Error(String("FAILED: OCIAttrSet(): setting attribute <username> in the authentication handle failed (")
						 << strErr << ")");
		return false;
	}

	if (checkError(OCIAttrSet(fUsrhp.getHandle(), (ub4)OCI_HTYPE_SESSION, (dvoid *)password,
							  (ub4)strlen((const char *)password), (ub4)OCI_ATTR_PASSWORD, fErrhp.getHandle()),
				   strErr)) {
		SystemLog::Error(String("FAILED: OCIAttrSet(): setting attribute <password> in the authentication handle failed (")
						 << strErr << ")");
		return false;
	}

	if (checkError(
			OCISessionBegin(fSvchp.getHandle(), fErrhp.getHandle(), fUsrhp.getHandle(), OCI_CRED_RDBMS, (ub4)OCI_DEFAULT),
			strErr)) {
	...
	Trace("connected to oracle as " << strUsername)
		...
		if (checkError(OCIAttrSet(fSvchp.getHandle(), (ub4)OCI_HTYPE_SVCCTX, fUsrhp.getHandle(), (ub4)0, OCI_ATTR_SESSION,
								  fErrhp.getHandle()),
					   strErr)) {
...
bool OracleConnection::checkError(sword status) {
	...
	StatTrace(OracleConnection.checkError, "status: " << (long)status << " retcode: " << (bRet ? "true" : "false"),
...
bool OracleConnection::checkError(sword status, String &message) {
	...
	if (bError) {
		...
		StatTrace(OracleConnection.checkError, "status: " << (long)status << " message [" << message << "]",
				  coast::storage::Current());
	}
	return bError;
}
...
OracleStatementPtr OracleConnection::createStatement(String strStatement, long lPrefetchRows,
	...
	if (aObjType == OracleConnection::TYPE_UNK) {
		try {
			...
			Trace(String("prepared stored ")
				  << ((aObjType == OracleConnection::TYPE_FUNC) ? "function: " : "procedure: ") << strStatement);
...
OracleConnection::ObjectType OracleConnection::DescribeObjectByName(const String &command, DscHandleType &aDschp,
																	OCIParam *&parmh) {
	...
	sword attrStat = OCIDescribeAny(SvcHandle(), ErrorHandle(), (dvoid *)objptr, (ub4)strlen((char *)objptr), OCI_OTYPE_NAME, 0,
									OCI_PTYPE_UNK, aDschp.getHandle());
	Trace("status after DESCRIBEANY::OCI_PTYPE_UNK: " << (long)attrStat) if (checkError(attrStat, strErr)) {
		strErr << "{" << command << " is neither a stored procedure nor a function}";
		throw OracleException(*this, strErr);
	}

	Trace("get the parameter handle") attrStat =
		OCIAttrGet(aDschp.getHandle(), OCI_HTYPE_DESCRIBE, (dvoid *)&parmh, 0, OCI_ATTR_PARAM, ErrorHandle());
...
OracleConnection::ObjectType OracleConnection::ReadSPDescriptionFromDB(const String &command, Anything &desc) {
	...
	if (aStmtType == TYPE_SYN) {
		Trace(
			"as we identified a synonym, we need to collect the scheme name and the referenced object name to ask for "
			"description");
		text *name(0);
		ub4 namelen(0);
		String strSchemaName;
		attrStat =
			OCIAttrGet((dvoid *)parmh, OCI_DTYPE_PARAM, (dvoid *)&name, (ub4 *)&namelen, OCI_ATTR_SCHEMA_NAME, ErrorHandle());
		if (checkError((attrStat))) {
			throw OracleException(*this, attrStat);
		}
	...
	Trace("get the number of arguments and the arg list for stored " << (bIsFunction ? "function" : "procedure"))
		OCIParam *arglst(0);
	ub2 numargs = 0;
	if (checkError((attrStat = OCIAttrGet((dvoid *)parmh, OCI_DTYPE_PARAM, (dvoid *)&arglst, (ub4 *)0, OCI_ATTR_LIST_ARGUMENTS,
										  ErrorHandle())))) {
		throw OracleException(*this, attrStat);
	}
	if (checkError((attrStat = OCIAttrGet((dvoid *)arglst, OCI_DTYPE_PARAM, (dvoid *)&numargs, (ub4 *)0, OCI_ATTR_NUM_PARAMS,
										  ErrorHandle())))) {
		throw OracleException(*this, attrStat);
	}
	...
	for (int i = start; i < end; ++i) {
		if (checkError((attrStat = OCIParamGet((dvoid *)arglst, OCI_DTYPE_PARAM, ErrorHandle(), (dvoid **)&arg, (ub4)i)))) {
			throw OracleException(*this, attrStat);
		}
		namelen = 0;
		name = 0;
		...
		if (checkError((attrStat = OCIAttrGet((dvoid *)arg, OCI_DTYPE_PARAM, (dvoid *)&dtype, (ub4 *)0, OCI_ATTR_DATA_TYPE,
											  ErrorHandle())))) {
			throw OracleException(*this, attrStat);
		}
		Trace("Data type: " << dtype)

			if (checkError((attrStat = OCIAttrGet((dvoid *)arg, OCI_DTYPE_PARAM, (dvoid *)&name, (ub4 *)&namelen, OCI_ATTR_NAME,
												  ErrorHandle())))) {
			throw OracleException(*this, attrStat);
		}
		...
		if (checkError((attrStat = OCIAttrGet((dvoid *)arg, OCI_DTYPE_PARAM, (dvoid *)&iomode, (ub4 *)0, OCI_ATTR_IOMODE,
											  ErrorHandle())))) {
			throw OracleException(*this, attrStat);
		}
...
String OracleConnection::ConstructSPStr(String const &command, bool pIsFunction, ROAnything desc, const String &strReturnName) {

...
public:
	...
	bool isOpen() const { return fStatus == eSessionValid; }
	 ...
	 */
	OracleEnvironment &getEnvironment() const { return fOracleEnv; }
	 ...
	 * @param lPrefetchRows How many rows to fetch in a server round trip, can be used to optimize performance on a per
	 * statement basis
	 * @param roaSPDescription Needs to be supplied if the statement is a stored procedure or function. This data is needed to
	 * supply the correct parameter names and types.
	 * @return newly created OracelStatment in case of success
	 */
	OracleStatementPtr createStatement(String strStatement, long lPrefetchRows,
									   OracleConnection::ObjectType aObjType = OracleConnection::TYPE_SIMPLE,
									   String strReturnName = String());

	/*! access OCIError handle
	 *
	 * @return OCIError handle
	 */
	OCIError *ErrorHandle() { return fErrhp.getHandle(); }

	/*! access OCISvcCtx handle
	 *
	 * @return OCISvcCtx handle
	 */
	OCISvcCtx *SvcHandle() { return fSvchp.getHandle(); }
	...
	bool AllocateHandle(handlePtrType &aHandlePtr) {
		if (OCIHandleAlloc(fOracleEnv.EnvHandle(), aHandlePtr.getVoidAddr(), aHandlePtr.getHandleType(), (size_t)0,
						   (dvoid **)0) != OCI_SUCCESS) {
			SystemLog::Error(String("FAILED: OCIHandleAlloc(): alloc handle of type ")
							 << (long)aHandlePtr.getHandleType() << " failed");
			return false;
		}
		return true;
	}

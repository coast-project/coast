...
OracleDAImpl::OracleDAImpl(const char *name) : DataAccessImpl(name) {
...
void OracleDAImpl::ProcessResultSet(OracleResultset &aRSet, ParameterMapper *&in, Context &ctx, ResultMapper *&out,
							...
							switch (status) {
								...
								case OracleStatement::UPDATE_COUNT_AVAILABLE: {
									...
									for (long lRowIdx = 0; lRowIdx < lIterations; ++lRowIdx) {
										...
										Context::PushPopEntry<Anything> aRowIndexEntry(
											ctx, "ResultRowIndex", anyRowIndex,
											(bIsArrayExecute ? "_OracleArrayResultIndex_" : "Guguseli"));
										...
										while (aDescIter.Next(aDescEl)) {
											...
											if (lColType == SQLT_CUR || lColType == SQLT_RSET) {
												...
												Trace("value of column [" << aDescEl.AsString("Name") << "] has value ["
																		  << anyValueCol.AsString("NULL") << "]");
...
Anything OracleDAImpl::getMappedInputValues(ParameterMapper *pmapIn, OracleStatement &aStmt, Context &ctx,
	...
	for (long lIdx = 0; lIdx < lIterations; ++lIdx) {
		...
		AnyExtensions::Iterator<OracleStatement::Description, OracleStatement::Description::Element> aDescIter(
			aStmt.GetOutputDescription());
		OracleStatement::Description::Element aDescEl;
		while (aDescIter.Next(aDescEl)) {
			long lRowLength = aDescEl.AsLong("MaxStringBufferSize", 0L);
			String strParamname(aDescEl.AsString("Name"));
			Anything anyValue;
			if (aDescEl.AsLong("IoMode") == (long)OCI_TYPEPARAM_IN || aDescEl.AsLong("IoMode") == (long)OCI_TYPEPARAM_INOUT) {
				...
				if (!pmapIn->Get(String("Params.").Append(strParamname), anyValue, ctx) && !bIsRSET) {
					throw OracleException(*(aStmt.getConnection()),
										  String("getMappedInputValues: In(out) parameter [")
											  << strParamname << "] not found in config, is it defined in upper case letters?");
				}

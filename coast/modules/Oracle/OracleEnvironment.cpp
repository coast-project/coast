...
dvoid *malloc_func(void *ctxp, size_t size) {
	...
	StatTrace(OracleEnvironment.mem_alloc,
			  "size: " << (long)size << " ptr: &" << (long)ptr
					   << " allocator:" << (pAlloc == coast::storage::Global() ? "global" : "local") << "&" << (long)pAlloc,
...
dvoid *realloc_func(void *ctxp, void *ptr, size_t size) {
	...
	StatTrace(OracleEnvironment.mem_realloc,
			  "size: " << (long)size << " oldptr: &" << (long)ptr << " new ptr: &" << (long)nptr
					   << " allocator:" << (pAlloc == coast::storage::Global() ? "global" : "local") << "&" << (long)pAlloc,
			  coast::storage::Current());
	return (nptr);
}
void free_func(void *ctxp, dvoid *ptr) {
	...
	StatTrace(OracleEnvironment.mem_free,
			  "ptr: &" << (long)ptr << " allocator:" << (pAlloc == coast::storage::Global() ? "global" : "local") << "&"
					   << (long)pAlloc,
			  coast::storage::Current());
	pAlloc->Free(ptr);
}

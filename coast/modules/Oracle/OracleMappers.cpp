...
OracleParameterMapper::OracleParameterMapper(const char *name) : ParameterMapper(name) {
...
OracleResultMapper::OracleResultMapper(const char *name) : ResultMapper(name) {
...
bool OracleResultMapper::DoPutAny(const char *key, Anything &value, Context &ctx, ROAnything script) {
	...
	if (lDelimIdx != -1) {
		...
		if (roaScript.IsNull()) {
			...
			Trace("key reduced to [" << strKey << "]") roaScript = SelectScript(strKey, script, ctx);
		}

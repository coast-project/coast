...
bool OracleStatement::AllocHandle() {
	...
	bool bSuccess(!fpConnection->checkError(
		OCIHandleAlloc(fpConnection->getEnvironment().EnvHandle(), fStmthp.getVoidAddr(), OCI_HTYPE_STMT, 0, 0), strError));
	if (!bSuccess) {
...
OracleStatement::Status OracleStatement::execute(ExecMode mode, long lIterations) {
	...
	if (fStatus == PREPARED) {
		...
		sword execStatus =
			OCIStmtExecute(fpConnection->SvcHandle(), getHandle(), fpConnection->ErrorHandle(), iters, 0, NULL, NULL, mode);
...
bool OracleStatement::Prepare() {
	...
	if (bSuccess) {
		if (!(bSuccess = !fpConnection->checkError(
				  OCIStmtPrepare(getHandle(), fpConnection->ErrorHandle(), (const text *)(const char *)fStmt,
								 (ub4)fStmt.Length(), OCI_NTV_SYNTAX, OCI_DEFAULT),
				  strErr))) {
...
void OracleStatement::setPrefetchRows(long lPrefetchRows) {
	...
	if (fpConnection->checkError(OCIAttrSet(getHandle(), OCI_HTYPE_STMT, &prefetch, sizeof(prefetch), OCI_ATTR_PREFETCH_ROWS,
											fpConnection->ErrorHandle()))) {
		fErrorMessages.Append(strErr);
	}
}
...
OracleResultsetPtr OracleStatement::getCursor(long lColumnIndex, long lRowIdx) {
	...
	if (lColumnIndex >= 0 && lColumnIndex < fDescriptions.GetSize()) {
		...
		Anything value;
		SubTrace(TraceColType,
				 "column type is: " << aDescEl.AsLong("Type") << " indicator: " << (long)aDescEl.getIndicatorValue(lRowIdx));
	...
	if (fDescriptions.GetSize() == 0L) {
		if (getStatementType() == STMT_SELECT) {
			...
			while (parm_status == OCI_SUCCESS) {
				...
				parm_status = OCIAttrGet((dvoid *)mypard, OCI_DTYPE_PARAM, (dvoid *)&data_len, 0, OCI_ATTR_DISP_SIZE, eh);
				if (parm_status != OCI_SUCCESS) {
					throw OracleException(*getConnection(), parm_status);
				}
				...
				parm_status =
					OCIAttrGet((dvoid *)mypard, OCI_DTYPE_PARAM, (dvoid **)&col_name, (ub4 *)&col_name_len, OCI_ATTR_NAME, eh);
				if (parm_status != OCI_SUCCESS) {
					throw OracleException(*getConnection(), parm_status);
				}
...
bool OracleStatement::DefineOutputArea() {
	...
	while (aDescIter.Next(aDescEl)) {
		...
		sword execStatus =
			OCIDefineByPos(pStmthp, &defHandle, eh, lColIndex, (void *)buf.AsCharPtr(), len, aDescEl.AsLong("Type"),
...
Anything OracleStatement::getValue(long lColumnIndex, long lRowIdx) {
	...
	if (lColumnIndex >= 0 && lColumnIndex < fDescriptions.GetSize()) {
		...
		SubTrace(TraceColType,
				 "column type is: " << aDescEl.AsLong("Type") << " indicator: " << (long)aDescEl.getIndicatorValue(lRowIdx));
		SubTrace(TraceBuf, "buf ptr " << (long)(aDescEl.getRawBufferPtr(lRowIdx))
									  << " length: " << (long)aDescEl.getEffectiveLengthValue());
		switch (aDescEl.AsLong("Type")) {
			case SQLT_INT:
				Trace("SQLT_INT");
			...
			default:
				...
				anyColValue = Anything(
					String(static_cast<void *>(aDescEl.getRawBufferPtr(lRowIdx)), (long)aDescEl.getEffectiveLengthValue()));
				break;
		}
	}
...
void OracleStatement::adjustColumnType(OracleStatement::Description::Element &aDescEl) {
	StartTrace1(OracleStatement.adjustColumnType,
				"column index:" << aDescEl.AsLong("Idx", -1L) << " type:" << aDescEl.AsLong("Type"));
	switch (aDescEl.AsLong("Type")) {
		case SQLT_DAT:
			Trace("SQLT_DAT");
			aDescEl["Length"] = 9;
...
sword OracleStatement::bindColumn(long lBindPos, OracleStatement::Description::Element &aDescEl, long len) {
	...
	OCIBind *bndp = 0;
	return OCIBindByPos(getHandle(), &bndp, getConnection()->ErrorHandle(), (ub4)lBindPos, (dvoid *)aDescEl.getRawBufferPtr(0L),
						(sword)len, aDescEl.AsLong("Type"), (dvoid *)aDescEl.getIndicatorBufferPtr(0L), 0, 0, 0, 0,
						OCI_DEFAULT);
}
...
namespace {
	void fillCBuffer(OracleStatement::Description::Element &aDescEl, char const *pcWhere, long len, long lRowIndex,
		...
		StatTrace(OracleStatement.fillRowColValue,
				  "adjust for row in buffer, before:" << (long)pC << " after:" << (long)(pC + (len * lRowIndex)),
...
void OracleStatement::fillRowColValue(OracleStatement::Description::Element &aDescEl, long lRowIndex,
									  ROAnything const roaValue) {

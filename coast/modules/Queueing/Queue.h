...
/*!
Queue elements are represented using either by objects of their type or Anythings. The internal queue itself is either a
std::container or an Anything which allows simple handling. Statistics can be made by evaluating the returned Anything from
GetStatistics(). The following slots are defined: <pre>
{
...
*/
template <class TElementType, class TListStorageType>
...
public:
	...
	template <typename T>
	QueueBase(const char *name, coast::typetraits::Type2Type<T>, size_type lQueueSize = std::numeric_limits<size_type>::max(),
	...
	QueueBase(const char *name, coast::typetraits::Type2Type<Anything>,
			  size_type lQueueSize = std::numeric_limits<size_type>::max(), Allocator *pAlloc = coast::storage::Global())
		: fName(name, -1, coast::storage::Global()),
		  fAllocator(pAlloc),
		  fQueueSize(lQueueSize),
		  fSemaFullSlots(0),
	...
	/*! Status code of queue Put and Get operations. As we support blocking of either read and/or write side of the queue, we
	 * must return appropriate codes telling the caller what happened. A simple boolean value is far not enough to find out why
	 * an operation failed. */
	enum StatusCode {
	...
	/*! Either blocking or non-blocking calls are possible by setting bTryLock appropriately. By default, blocking calls are
	  made. When the queue will get shut down, blocking callers will get released and informed by StatusCode::eBlocked
	...
	size_type capacity() const { return fQueueSize; }
	...
	void GetStatistics(Anything &anyStatistics) {
		...
		if (IsAlive()) {
			...
			anyStatistics["CreateTime"]["sec"] =
				static_cast<long>(DiffTimer::Scale(fQueueStartTime.getRawStartTime(), DiffTimer::eSeconds));
			anyStatistics["StatisticTime"]["sec"] =
				static_cast<long>(DiffTimer::Scale(DiffTimer::getCurrentRawTime(), DiffTimer::eSeconds));
			...
			anyStatistics["LifeTime"]["usec"] =
				static_cast<long>(DiffTimer::Scale(elapsedTimeSinceStart, DiffTimer::eMicroseconds) -
								  (secondsSinceStart * DiffTimer::eMicroseconds));
	...
	inline bool IsAlive() { return fAlive == 0xf007f007; }
	...
	//! Block the specified queue side (put/get) for access. If a call is made to a blocked queue side, the method will return
	//! immediately and set the return code to StatusCode::eBlocked.
	...
	String typeName() const {
		return coast::utility::demangle<String>(typeid(ListStorageType).name())
			.Append('[')
			.Append(coast::utility::demangle<String>(typeid(ElementType).name()))
			.Append(']');
	}
...
protected:
	...
	StatusCode DoGet(ElementTypeRef anyElement, coast::typetraits::Type2Type<T>) {
		...
		if (!IsBlocked(eGetSide)) {
			...
			if (fContainer.size()) {
				anyElement =
					fContainer.front();	 //! \todo change here so that it's not restricted only for Anything's - use any_cast
	...
	/*! To keep track of Put method callers, a LockedValueIncrementDecrementEntry will be used.
	  This is achieved by releasing the semaphore for any caller who currently entered the method which will then be able to
	  acquire the semaphore but then needs to exit the method due to its blocked state */
	...
	size_type fMaxLoad;
	long fBlockingPutCount, fBlockingGetCount;
	u_long fAlive;
...
public:
	...
	Queue(const char *name, size_type lQueueSize = std::numeric_limits<size_type>::max(),
		  Allocator *pAlloc = coast::storage::Global())

...
QueueWorkingModuleTest::QueueWorkingModuleTest(TString tstrName) : TestCaseType(tstrName) {
...
void QueueWorkingModuleTest::InitFinisNoModuleConfigTest() {
	...
	t_assertm(!aModule.Init(GetTestCaseConfig()["ModuleConfig"]),
			  "module init should have failed due to missing configuration");
}

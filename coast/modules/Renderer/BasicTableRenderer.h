...
protected:
	...
	virtual bool DoHandleAdditionalRenderers(ROAnything &elmt, ROAnything &fmt, Renderer **resultRenderer,
											 Anything &resultConfig);
	...
	void PrintBodySection(const ROAnything &config, int &row, std::ostream &reply, Context &c, const ROAnything &repBody,
						  const Anything &repData);
	void PrintRow(int &row, std::ostream &reply, Context &c, RowAccessor *accessors, long rowSize, ROAnything &rowColors,
				  bool invertHeaders = true);
	...
	virtual void DoPrintEmptySection(std::ostream &reply, Context &c, const ROAnything &config, const ROAnything &sectionConfig,
									 int &row);
	...
	RowAccessor *SetupRowAccessors(const ROAnything &conf, Context &c, const ROAnything &rowMeta, long *rowSize, bool isBold,
								   bool invertHeaders);

...
void DateRenderer::RenderAll(std::ostream &reply, Context &ctx, const ROAnything &config) {
	...
	if (config.LookupPath(roaFormat, "Format") ||
		((configSize > 0) && (config.SlotName(0) == 0) && config.LookupPath(roaFormat, ":0"))) {
	...
	if (config.LookupPath(roaCfg, "Date") ||
		((configSize > 1) && (config.SlotName(1) == 0) && config.LookupPath(roaCfg, ":1"))) {
	...
	if (config.LookupPath(roaOffset, "Offset") ||
		((configSize > 2) && (config.SlotName(1) == 0) && config.LookupPath(roaOffset, ":2"))) {

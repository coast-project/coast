...
The ImageRenderer renders an HTML \<IMG\> tag into the reply (including different HTML options). (The ImageLocationRenderer is
used to render the filename of the image.) All HTML image options may be specified at slot /Options, e.g.:
-# /DYNSRC "..."\n
...
-# /LOWSRC "..."\n
 Specifies the URL of an image to be loaded first, before the image specified in SRC is loaded. LOWSRC usually reefers to a
smaller image.
-# /ALIGN "..."\n
...
Renders an image with a caption (an HTML table is used instead of the regular HTML CAPTION feature which does not perform well
on all browsers) The renderer takes the same parameters as an ImageRenderer - i.e. 'ImagePath', 'ImageName' and 'Options'
(eventhough all 'Options' might not have the desired results, since the image is enclosed by a table). In addition the text to
be displayed below the image is taken from the 'Caption' slot. If 'Options' are not supplied explicitly, a border is drawn
around the image as a default. The caption-text will be used as ALT text for the image.
*/

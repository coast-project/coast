...
Loops over the slots of an Anything looked up in the context using the rendered ListName string. For every named or unnamed slot
of this list its content will be temporarily stored in a specified slot in the temp store. The EntryRenderer specification can
then use lookups to access this data. Additional Rendererspecs to add stylistic sugar like Headers and Footers can be added too.
...
public:
	ListRenderer(const char *name) : Renderer(name) {}

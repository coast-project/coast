...
SSL_SESSION *SSLObjectManager::GetSessionId(const String &ip, const String &port) {
	...
	SSL_SESSION *sslsess(0);
	{
		...
		if (fSSLSessionIdStore.LookupPath(anySess, storeId, storeIdDelim) && anySess["session"].AsIFAObject(0) != 0) {
			...
			Trace("Got SessionId for id " << storeId << " thread: " << thrId << " session: " << SessionIdAsHex(sslsess));
		}
	}
	return sslsess;
}
...
void SSLObjectManager::SetSessionId(const String &ip, const String &port, SSL_SESSION *sslSession) {
	...
	SSL_SESSION *sslSessionStored(0);
	{
		...
		if (fSSLSessionIdStore.LookupPath(anySess, storeId, storeIdDelim) && anySess["session"].AsIFAObject(0) != 0) {
			...
			Trace("Freeing old SessionId: " << SessionIdAsHex(sslSessionStored)
											<< " RefCount: " << sslSessionStored->references);
	...
	Trace("Set SessionId for id " << storeId << " thread: " << thrId << " session: " << SessionIdAsHex(sslSession));
}
			...
			for (long port = 0; port < fSSLSessionIdStore[ip].GetSize(); port++) {
				for (long thrId = 0; thrId < fSSLSessionIdStore[ip][port].GetSize(); thrId++) {
					if (fSSLSessionIdStore[ip][port][thrId]["session"].AsIFAObject(0) != (IFAObject *)NULL) {
						Trace("Freeing ssl session for address: "
							  << fSSLSessionIdStore.SlotName(ip) << " port: " << fSSLSessionIdStore[ip].SlotName(port)
							  << " thread: " << fSSLSessionIdStore[ip][port].SlotName(thrId) << " session: "
							  << SessionIdAsHex((SSL_SESSION *)fSSLSessionIdStore[ip][port][thrId]["session"].AsIFAObject(0)));

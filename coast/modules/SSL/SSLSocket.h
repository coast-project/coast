...
class SSLSocketArgs {
public:
	SSLSocketArgs(bool verifyCertifiedEntity, const String &certVerifyString, bool certVerifyStringIsFilter,
...
public:
	...
	virtual X509 *GetPeerCert() { return fPeerCert; }
...
class SSLClientSocket : public SSLSocket {
	...
	// and closes it in destructor
public:
	SSLClientSocket(SSL_CTX *ctx, int socket, const Anything &clientInfo, bool doClose = true, long timeOut = 0L,
					Allocator *a = coast::storage::Global());
	SSLClientSocket(SSLSocketArgs sslSocketArgs, SSL_CTX *ctx, int socket, const Anything &clientInfo, bool doClose = true,
					long timeOut = 0L, Allocator *a = coast::storage::Global());
...
public:
	SSLServerSocket(SSL_CTX *ctx, int socket, const Anything &clientInfo, bool doClose = true,
					long timeOut = 300 * 1000 /* 5 minutes */,
					Allocator *a = coast::storage::Global());  // use socket descriptor
...
public:
	...
	SSLConnector(const char *ipAdr, long port, long connectTimeout = 0, SSL_CTX *ctx = 0, const char *srcIpAdr = 0,
				 long srcPort = 0, bool threadLocal = false);
	//! If no SSL_CTX  is provided by the caller, a default client SSL_CTX will be created.
	SSLConnector(ConnectorArgs &connectorArgs, SSL_CTX *ctx = NULL, const char *srcIpAdr = 0, long srcPort = 0,
				 bool threadLocal = false);
	//! If no SSL_CTX is provided by the caller and the sslModuleCfg  is NULL, a new default client SSL_CTX will be created.
	//! If SSL_CTX  is NULL and sslModuleCfg is not NULL,  a new client SSL_CTX is created considering the sslModuleCfg
	//! settings. In both cases the created SSL_CTX is stored by SSLObjectManager for later reuse. If you pass in a SSL_CTX (not
	//! NULL) this SSL_CTX is used. It will not be deleted. Use this  method when you need a specialized SSL_CTX which
	//! corresponds  to your SSLSocketArgs
	SSLConnector(ConnectorArgs &connectorArgs, SSLSocketArgs sslSocketArgs, ROAnything sslModuleCfg = ROAnything(),
				 SSL_CTX *ctx = NULL, const char *srcIpAdr = 0, long srcPort = 0, bool threadLocal = false);

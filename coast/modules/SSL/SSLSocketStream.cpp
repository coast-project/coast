...
iosITOSSLSocket::iosITOSSLSocket(SSL *ctx, SSLSocket *ssl, long timeout)
	: fSSLSockBuf(ctx, ssl, timeout), fAllocator(ssl->GetAllocator()) {
	init(&fSSLSockBuf);
}
...
long SSLSocketStreamBuf::DoWrite(const char *buf, long len) {
	...
	} while (bytesSent < 1 && len > 0 && ((SSLSocket *)fSocket)->ShouldRetry(fContext, bytesSent, false));
#ifdef STREAM_TRACE
	if (bytesSent > 0) {
...
long SSLSocketStreamBuf::DoRead(char *buf, long len) const {
	...
	} while (bytesRead < 1 && len > 0 && ((SSLSocket *)fSocket)->ShouldRetry(fContext, bytesRead, false));
#ifdef STREAM_TRACE
	if (bytesRead > 0) {

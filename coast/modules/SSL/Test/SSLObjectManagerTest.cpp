...
SSLObjectManagerTest::SSLObjectManagerTest(TString tstrName) : TestCaseType(tstrName) {
...
void SSLObjectManagerTest::UsePassedInCtxTest() {
	...
	while (aEntryIterator.Next(cConfig)) {
		...
		for (long ii = 0; ii < cConfig["Config"]["Runs"].AsLong(10); ii++) {
			...
			assertEqual(cConfig["Results"]["SSLCertVerifyStatus"].AsBool(1),
						clientInfo["SSL"]["Peer"]["SSLCertVerifyStatus"]["SSL"]["Ok"].AsBool(0));
			assertEqual(cConfig["Results"]["AppLevelCertVerifyStatus"].AsBool(1),
						clientInfo["SSL"]["Peer"]["AppLevelCertVerifyStatus"].AsBool(0));
...
void SSLObjectManagerTest::SessionIdTest() {
	StartTrace(SSLObjectManagerTest.SessionIdTest);
	{
		...
		while (aEntryIterator.Next(cConfig)) {
			...
			SSL_SESSION *sslSessionResult =
				SSLObjectManager::SSLOBJMGR()->GetSessionId(cConfig["Address"].AsString(), cConfig["Port"].AsString());

...
Connector *ConnectorDAImpl::DoMakeConnector(Context &context, ParameterMapper *in, ResultMapper *out) {
	...
	lConnTimeout *= 1000L;
	Trace("Address<" << strAddress << "> Port[" << lPort << "] " << (bUseSSL ? "SSL" : "")
					 << " ConnectTimeout: " << lConnTimeout << "ms");
	if (bUseSSL) {
	...
	if (socketFd != 0L) {
		...
		if (bSuccess && error == 0) {
			...
			if (roaParams["HTTPS"].AsBool() || bUseSSL) {
				s = new (coast::storage::Global())
					SSLClientSocket((SSL_CTX *)context.Lookup("SSLContext").AsIFAObject(0), socketFd, anyParams, true);
			} else {
...
bool ConnectorDAImpl::SendInput(std::iostream *pIos, Socket *s, long lCheckReadyTimeout, long lSocketStreamTimeout,

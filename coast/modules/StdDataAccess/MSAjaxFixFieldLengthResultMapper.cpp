...
namespace {
	...
	String getSubString(String const &sText, long const valueFieldLength) {
		char const *itStart = sText.cstr(), *it = itStart, *itEnd = itStart + sText.Length();
		try {
		...
		StatTrace(MSAjaxFixFieldLengthResultMapper.getSubString,
				  "len: " << valueFieldLength << " bytelen: " << (long)(it - itStart) << " str [" << valueString << "]",
	...
	long getStringLength(String const &str) {
		...
		StatTrace(MSAjaxFixFieldLengthResultMapper.getStringLength, "len: " << len << " str [" << str << "]",
				  coast::storage::Current());
		return len;
	}

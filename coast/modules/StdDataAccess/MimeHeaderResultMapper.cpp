...
bool MimeHeaderResultMapper::DoPutStream(const char *key, std::istream &is, Context &ctx, ROAnything script) {
	...
	coast::urlutils::NormalizeTag shift =
		static_cast<coast::urlutils::NormalizeTag>(Lookup(normalizeKey, static_cast<long>(coast::urlutils::eUpshift)));
	MIMEHeader mh(shift);
	try {
		...
		coast::http::PutErrorMessageIntoContext(ctx, 413, String(e.what()).Append(" => check setting of [LineSizeLimit]"),
												e.fLine, "MimeHeaderResultMapper::DoPutStream");
	}
	return false;
}

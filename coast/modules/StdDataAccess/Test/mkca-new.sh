...
if [ "$serialnum" = "%s" ]; then
	...
	serialnum=$(( ( ( $(date +%Y) - 1970 ) * 365 + ( $(date +%Y) - 1970 ) / 4 - 1 + $(date +%j) ) * 86400 + $(date +%H) * 3600 + $(date +%M) * 60 + $(date +%S) ))
fi
...
showhelp()
{
    ...
    echo " -s <digest>      : message digest used to sign server/client certificate. choose one of: md2/md5/sha1/mdc/mdc2, default is $digest"
    echo " -e <emailsuffix> : email suffix to use, default is $emailsuffix"
    echo ''
    exit 3
}
...
if [ ! -x "${openssl_bin}" ]
then
	showhelp "Could not find openssl binary."
fi
...
if [ ! -f "$conf" ]
then
    showhelp "Could not find openssl configuration file ${conf}"
fi
...
if [ -z "${target_dir}" ]
then
	showhelp "target directory not set, exiting!"
fi
...
cd "${target_dir:?}" || { "Cannot change to directory ${target_dir}, exiting!"; exit 4; }
...
mkdir subCA1
cd subCA1 || { echo "Failed to create subCA1, exiting!"; exit 5; }
...
cd "${target_dir:?}"/ucerts || { echo "Failed to change directory to ${target_dir:?}/ucerts, exiting!"; exit 6; }
for commonnameserver_var in ${commonnameserver}
do

...
//---- SybCTnewDAImplTest ----------------------------------------------------------------
SybCTnewDAImplTest::SybCTnewDAImplTest(TString tstrName) : TestCaseType(tstrName), fbWasInitialized(false) {
...
void SybCTnewDAImplTest::UninitializedExecTest() {
	...
	t_assertm(da.Exec(ctx, &inpMapper, &outMapper) == false,
			  "DataAccess should have failed due to uninitialized SybCTnewDAImpl!");
...
void SybCTnewDAImplTest::DoPutbackConnectionTest() {
	...
	if (t_assert(SybCTnewDAImpl::Init(anyCfg))) {
		...
		t_assertm(TimeStamp(SybCTnewDAImpl::fgListOfSybCT["Open"].SlotName(0L)) <
					  TimeStamp(SybCTnewDAImpl::fgListOfSybCT["Open"].SlotName(1L)),
				  "expected timestamps to be different");
		t_assert(SybCTnewDAImpl::Finis());
	}
}

...
#ifdef TRACE_LOCK_UNLOCK
#define UNLOCKMUTEX(mutex, retcode)                 \
	if (!::ReleaseMutex(mutex))                     \
		cerr << "error in releasing mutex" << endl; \
	fooRELOBJECT(mutex);
#else
...
#ifdef TRACE_LOCK_UNLOCK
#define UNLOCKSEMA(sema)                                \
	if (!::ReleaseSemaphore(sema, 1, NULL))             \
		cerr << "error in releasing semaphore" << endl; \
	fooRELOBJECT(sema);
#else
...
#ifdef TRACE_LOCK_UNLOCK
#define UNLOCKRWLOCK(rwlock, r)                             \
	if (!::ReleaseMutex(rwlock))                            \
		cerr << "error in releasing rwlock(mutex)" << endl; \
	fooRELOBJECT(rwlock);
#else
...
#define GETTLSDATA(key, data, dataType) \
	((data = (dataType *)TlsGetValue(key)), (GetLastError() != NO_ERROR ? (data = 0) : data))

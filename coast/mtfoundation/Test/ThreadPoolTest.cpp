...
public:
	...
	GugusThread(const char *name, Allocator *a) : tBaseClass(name, a) {}
...
public:
	...
	/*! does nothing, real work is done in init, object is still unusable without call of Init() method!
	  \param name Give the pool Manager a name, gets printed in statistics */
	TestPoolManager(const char *name) : tBaseClass(name){};
	...
	void Update(tObservedPtr pObserved, tArgsRef aUpdateArgs) { fanyUpdates.Append(aUpdateArgs.DeepClone()); }

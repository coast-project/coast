...
/*!
A thread pool can be used in cases where we do not need to know what each thread does or when it does anything. Important is,
that we can have an amount of parallel workers running and waiting on something to do. <p>The thing is that we do not need an
element of control which gets work from somewhere, selects a free Thread and lets it process the work. The Thread can do this on
its own. Just implement a special kind of Thread which does exactly this. An excellent usage could be processing elements from a
queue.
*/
...
public:
	...
	virtual int Start(bool usePoolStorage, int poolStorageSize, int numOfPoolBucketSizes,
					  ROAnything roaThreadArgs = ROAnything());
	...
	//! Set my name
	void SetName(const char *str) { fName = str; }

	bool InTermination() const { return fTerminated; }
...
protected:
	...
	/*! access configuration for the ith thread, overwrite this method if you do not want to use roaThreadArgs[i] as the threads
	  specific arguments when initializing it
	...
	/*! access configuration for the ith thread, overwrite this method if you do not want to use roaThreadArgs[i] as the threads
	  specific arguments when starting it
	  \param i index of the thread to get the configuration for, poolSize > i >=0
	  \param roaThreadArgs ROAnything carrying thread specific information
	  \return roaThreadArgs[i] or an empty ROAnything */
...
After processing the workload it sets its running state back to ERunningState::eReady and waits until it gets selected by the
WorkerPoolManager again
*/
...
protected:
	//! implements the worker cycle<br><ul><li>waiting for work<li>processing assigned work<li>checking back and waiting
	//! again</ul><br> until terminated
	void Run();

	/*! used for general initialization of the worker when it is created. Arbitrary parameters may be passed using the
	  ROAnything...
	...
	/*! if we use pool allocators to allocate memory they will be refreshed in DoReadyHook(). Take care in implementing Workers
	  which optimize for memory speed! set to true if a refresh is possible, to false if we should not refresh it */
...
class PoolManager : public StatGatherer, public Observable<TObservedType, TArgs>::Observer {
public:
...
public:
	...
	int Init(int maxParallelRequests, int usePoolStorage, int poolStorageSize, int numOfPoolBucketSizes,
	...
	/*! critical region entry to process the next work package.
	  This method blocks the caller (e.g. the server accept-loop) if the pool has no worker ready,  i.e. ResourcesUsed() ==
	  GetPoolSize()
...
protected:
	...
	/*! finds a runnable WorkerThread object. Since there are fPoolSize WorkerThread objects and also fPoolSize allowed in the
	  critical region there should always at least one be runnable

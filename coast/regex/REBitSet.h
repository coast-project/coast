...
class REBitSet {
	...
	uint32_t &Bucket(unsigned char index) { return fSet[index / fgcSubSetBits]; }
	const uint32_t &Bucket(unsigned char index) const { return fSet[index / fgcSubSetBits]; }
...
public:
	...
	bool operator==(const REBitSet &b) const { return IsEqual(b); }
	bool operator<=(const REBitSet &b) const { return IsSubSet(b); }
	...
	REBitSet &Complement();
	//! test the bit
	bool IsMember(unsigned char bit) const { return ((Bucket(bit) & Slot(bit)) != 0U); }
	...
	static uint32_t Bit(unsigned char bit) { return Slot(bit); }
	static bool IsMember(uint32_t bits, unsigned char bit) { return ((bits & Slot(bit)) != 0U); }
	static uint32_t Slot(unsigned char index) { return fgcSingletons[index & fgcSubSetMask]; }

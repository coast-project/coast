...
public:
	...
	typedef typename boost_or_std::conditional<boost_or_std::is_same<PlainType, ROAnything>::value, ROAnything,
											   _ParameterType>::type ParameterType;
	...
	explicit AnyLookupInterfaceAdapter(ParameterType container, const char *pcBaseKey = NULL)
		: fContainer(container), fstrBaseKey(pcBaseKey) {}
	...
	virtual bool DoLookup(const char *key, ROAnything &result, char delim, char indexdelim) const {
		...
		if (fstrBaseKey.Length() != 0) {
			if (strKey.StartsWith(fstrBaseKey)) {
				...
				Trace("given key [" << strKey << "] does not start with [" << fstrBaseKey
									<< "], content not found and returning false");
				return false;
			}
		}
...
public:
	...
	ROAnyConfNamedObjectLookupAdapter(ROAnything ro, ConfNamedObject *cfno) : BaseClassType(ro) { fCfno = cfno; }

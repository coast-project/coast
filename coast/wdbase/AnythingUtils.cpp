...
void StoreFinder::Operate(Context &context, Anything &dest, const ROAnything &config) {
	...
	if (!dest.SetAllocator(anyStore.GetAllocator())) {
		SYSWARNING(
			"Tried to set allocator on Anything having an Impl already! Keep in mind that you might be operating on a copy!");
	}
	...
	SlotFinder::Operate(anyStore, dest, destSlotname, config["Delim"].AsCharPtr(".")[0L],
...
void StorePutter::Operate(Anything &source, Context &c, const ROAnything &config) {
	...
	SlotPutter::Operate(source, StoreFinder::FindStore(c, strStoreName), Renderer::RenderToString(c, config["Slot"]),
						config["Append"].AsBool(false), config["Delim"].AsCharPtr(".")[0L],
						config["IndexDelim"].AsCharPtr(":")[0L]);
}

void StorePutter::Operate(Anything &source, Context &c, String strStoreName, String destSlotname, bool append, char delim,
						  char indexdelim) {
	StatTrace(StorePutter.Operate,
			  "putting in store [" << (strStoreName.Length() ? (const char *)strStoreName : "TmpStore") << "] slot ["
								   << destSlotname << "]",

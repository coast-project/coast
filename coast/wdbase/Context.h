...
public:
	...
	public:
		...
		/* Constructor of PushPopEntry. An object of this class can be used where some content must be temporarily pushed onto
		  the Contexts lookup stack. The current implementation uses AnyLookupInterfaceAdapter to wrap the given [RO]Anything.
		...
		~PushPopEntry() {
			if (fbPushSuccess) {
				...
				if (!bPopCode) {
					...
					SYSWARNING("names of pushed [" << fStoreName << "] and popped [" << strPoppedStore
												   << "] Stores were not identical!");
				}
			}
		}
	...
	/*! Replace the first registerable object named by <I>key</I> with <I>li</I>. If there is no object named by key in the
	  Context li is pushed with key.
	  \param key only a simple key can match
	  \param li the new object */
	void Replace(const char *key, LookupInterface *li);

	/*! Remove the first registerable object named by <I>key</I>. If there is no object named by key in the Context it is
	  removed
	  \param key only a simple key can match
...
//--- legacy api starts here ---//
	...
	void SetLanguage(const char *lang) { fLanguage = lang; }

	//! returns the user id if any of this request
	const char *User() const { return fUser; }
...
private:
	/*! Push the given Anything on top of the lookup stack. This is a very convenient way to pass arguments to subsequent
	...
	/*! if set, make a copy of the session store. This allows concurrent requests using the same session because lookups
	 * targeting the session store don't need a lock. */
	...
	Anything &GetRoleStore() { return GetRoleStoreGlobal(); }
...
public:
	SessionReleaser(Context &c) : fContext(c) { fWasLockedByMe = fContext.UnlockSession(); }
	virtual ~SessionReleaser() {
		if (fWasLockedByMe) {
			fContext.LockSession();
		}
	}
	void Use() const { /* dummy */
	}

...
bool RegisterableObject::Reset(const ROAnything installerSpec, const char *category, InstallerPolicy *installer,
...
ConfNamedObject *ConfNamedObject::ConfiguredClone(const char *category, const char *name, bool bInitializeConfig) {
	StartTrace1(ConfNamedObject.ConfiguredClone,
...
bool ConfNamedObject::DoUnloadConfig() {
	StartTrace1(ConfNamedObject.DoUnloadConfig,
				"cat <" << fCategory << "> fName <" << fName << "> &" << (long)(IFAObject *)this);
...
bool ConfNamedObject::DoLookup(const char *key, ROAnything &result, char delim, char indexdelim) const {
	StartTrace1(ConfNamedObject.DoLookup, "key: <" << NotNull(key) << ">"
												   << " Name: <" << fName << ">");
	bool bSuccess(IsConfigLoaded());
	if (!bSuccess) {
		SystemLog::Warning(String("ConfNamedObject::DoLookup: key [")
						   << NotNull(key) << "] failed, object <" << fName << "> of registry category <" << fCategory
						   << "> not initialized!\n");
	} else {
...
bool HierarchConfNamed::DoLookup(const char *key, class ROAnything &result, char delim, char indexdelim) const {
	StartTrace1(HierarchConfNamed.DoLookup, "key: <" << NotNull(key) << ">"
													 << " Name: <" << fName << ">");

...
namespace {
	...
	long const lookupNameIndex = 0L, defaultIndex = 1L, delimIndex = 2L, indexDelimIndex = 3L;
}  // namespace
...
ROAnything ContextLookupRenderer::DoLookup(Context &context, const char *name, char delim, char indexdelim) {
	...
	StatTraceAny(ContextLookupRenderer.DoLookup, roaRet, "specification for [" << NotNull(name) << "]",
...
ROAnything StoreLookupRenderer::DoLookup(Context &context, const char *name, char delim, char indexdelim) {
	...
	StatTraceAny(StoreLookupRenderer.DoLookup, roaRet, "specification for [" << NotNull(name) << "]",
				 coast::storage::Current());
	return roaRet;
}
...
ROAnything QueryLookupRenderer::DoLookup(Context &context, const char *name, char delim, char indexdelim) {
	...
	StatTraceAny(QueryLookupRenderer.DoLookup, roaRet, "specification for [" << NotNull(name) << "]",
				 coast::storage::Current());
	return roaRet;
}

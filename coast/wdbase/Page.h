...
public:
	PagesModule(const char *name) : WDModule(name) {}
...
//! Render normally uses the Renderer class to generate page content. Although it is possible to overwrite hook methods in
//! subclasses, it should not be necessary since almost everything is configurable.
...
protected:
	...
	//! renders protocol body using "PageLayout" tag or subclass api sequence of Header, Title, Body, Footer to render HTML
	//! content
...
private:
	...
	virtual bool Postprocess(String &action, Context &c) { return false; }
	//! subclass hook to implement preprocessing; legacy, use actions instead
	virtual bool DoPrepare(String &transition, Context &c) { return false; }
};

...
public:
	...
	/*! Installs objects into registry driven by installerSpec using InstallerPolicy ip. It creates registry entries according
	  to the installer policy passed in.
	...
	//! installs the object o under name in the registry; after that in can be retrieved with Find. No checks against duplicate
	//! installs are made
	...
	//! removes the object associated with name (if any) from the registry, it does not delete anything. After that it is no
	//! longer accessible under name
	//! \param name name used for registration

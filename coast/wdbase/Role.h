...
public:
	RolesModule(const char *name) : WDModule(name) {}
...
public:
	...
	Role(const char *name) : HierarchConfNamed(name) {}
   ...
   \return true if pagename is set, false if transition couldn't find a page */
	...
	//! copies information from query or query["fields"] into the tmp store; if they are not already defined there; this takes
	//! place before processing the request
	virtual void PrepareTmpStore(Context &c);  // copies state contained in the query to the TmpStore
	//! copies information from the tmp store into the query according to a <i>StateFull</i> list defined in the role's
	//! configuration
	...
	//! hook to synchronize the rolestore with the required content to process the query at hand; default implementation does
	//! nothing
	...
	/*! return the session timeout value for this role
	 This is retrieved from the roles configuration slot "SessionTimeout", if not configured the default is 60 seconds.
	...
	String GetRequestRoleName(Context &ctx, String const &transition) const { return DoGetRequestRoleName(ctx, transition); }
	/*! Get a number of the given Role's hierarchy depth, based on the configured hierarchy
	 * @param pRole Role to get the level for
	 * @return number which tells how many hierarchy levels we are away from the top level Role, or -1 if given Role is not
	 * valid
	 */
...
protected:
	...
	IFAObject *Clone(Allocator *a) const { return new (a) Role(fName); }

...
public:
	ServiceDispatchersModule(const char *name) : WDModule(name) {}
...
public:
	...
	ServiceDispatcher(const char *serviceDispatcherName) : HierarchConfNamed(serviceDispatcherName) {}
	//! registry interface
	RegCacheDef(ServiceDispatcher);
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) ServiceDispatcher(fName); }
...
//! the uri prefix of the request is mapped to a service name using a map defined in the context with the tag
//! <b>URIPrefix2ServiceMap</b><br> the service name can be a renderer specification<br> if nothing matches the
//! <b>DefaultService</b> Entry is used
...
public:
	RendererDispatcher(const char *rendererDispatcherName) : ServiceDispatcher(rendererDispatcherName) {}
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) RendererDispatcher(fName); }

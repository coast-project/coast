...
public:
	SessionFactoriesModule(const char *name) : WDModule(name) {}
...
public:
	SessionFactory(const char *SessionFactoryName) : HierarchConfNamed(SessionFactoryName) {}
	RegCacheDef(SessionFactory);
	/*! @copydoc IFAObject::Clone(Allocator *) const */
	IFAObject *Clone(Allocator *a) const { return new (a) SessionFactory(fName); }
...
public:
	...
	//! Retrieves information about the session list and stores the information in slot \b SessionInfo in the TmpStore, locks
	//! fSessionsMutex
...
protected:
	...
	/*! This method tries to find a registered URLFilter by looking up its name in the Context at \b URLFilterName. If no name
	 * is set, the default URLFilter is used. The URLFilter configuration must be defined in the Context as it gets looked up
	 * using the name \b URLFilterSpec Check @ref URLFilterHandleQueryConfiguration to see how to configure.
	 *

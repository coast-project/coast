	...
	namespace testframework {
		...
		void CheckStores(Anything &anyFailures, ROAnything expected, Context &ctxToCheck, const char *testCaseName,
			...
			if (expected.IsDefined("SessionStore")) {
				CheckStoreContents(anyFailures, ctxToCheck.GetSessionStore(), expected["SessionStore"], "SessionStore",
								   testCaseName, delimSlot, delimIdx, rct);
			}
			if (expected.IsDefined("RoleStore")) {
				CheckStoreContents(anyFailures, ctxToCheck.GetRoleStoreGlobal(), expected["RoleStore"], "RoleStore",
								   testCaseName, delimSlot, delimIdx, rct);
			}
			if (expected.IsDefined("Query")) {
				CheckStoreContents(anyFailures, ctxToCheck.GetQuery(), expected["Query"], "Query", testCaseName, delimSlot,
								   delimIdx, rct);
			}
			if (expected.IsDefined("TmpStore")) {
				CheckStoreContents(anyFailures, ctxToCheck.GetTmpStore(), expected["TmpStore"], "TmpStore", testCaseName,
								   delimSlot, delimIdx, rct);
			}
		}
	}  // namespace testframework
}  // namespace coast

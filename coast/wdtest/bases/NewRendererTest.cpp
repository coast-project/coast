...
void NewRendererTest::TestCases() {
	...
	if (t_assertm(!GetTestCaseConfig().IsNull(), "no Tests configured!")) {
		...
		TraceAny(GetConfig()["RunOnly"], "run only config") while (aEntryIterator.Next(roaCaseConfig)) {
			TString slotToCheck;
			...
			if ((GetConfig()["RunOnly"].GetSize() == 0L) || GetConfig()["RunOnly"].Contains((const char *)slotToCheck)) {
				...
				Context c(GetConfig()["EnvForAllCases"].DeepClone(), roaCaseConfig["Env"].DeepClone(), theServer, &theSession,
						  theRole, thePage);

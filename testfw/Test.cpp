...
bool Test::assertImplementation(bool condition, TString conditionExpression, long lineNumber, TString fileName) {
	return assertImplementation(condition, conditionExpression, lineNumber, fileName, "");
...
bool Test::assertEquals(long expected, long actual, long lineNumber, TString fileName) {
	return assertEquals(expected, actual, lineNumber, fileName, "");
}  // assertImplementation

bool Test::assertEquals(long expected, long actual, long lineNumber, TString fileName, TString message) {
	return assertImplementation((expected == actual), notEqualsMessage(expected, actual), lineNumber, fileName, message);
}

/* check for a failed equality assertion for char* */
bool Test::assertEquals(const char *expected, const char *actual, long lineNumber, TString fileName) {
	return assertEquals(expected, actual, lineNumber, fileName, "");
}  // assertEquals
...
bool Test::assertEqualsIfNullPtr(const char *expected, const char *actual, long lineNumber, TString fileName, TString message) {
	if (expected == 0) {
...
/* check for a failed equality assertion */
bool Test::assertEquals(double expected, double actual, double delta, long lineNumber, TString fileName) {
	return assertEquals(expected, actual, delta, lineNumber, fileName, "");
}  // assertEquals
bool Test::assertEquals(double expected, double actual, double delta, long lineNumber, TString fileName, TString message) {
	return assertImplementation((fabs(expected - actual) < delta), notEqualsMessage(expected, actual), lineNumber, fileName,
								message);
}
...
/* build a message about a failed equality check */
TString Test::notEqualsMessage(const char *expected, long lengthExpected, const char *actual, long lengthActual) const {
	TString message;
	// evaluate difference and show it; useful for very long and complicated strings

...
public:
	...
	const char *getClassName() { return fClassName; }
	Test *getSuite() { return this; }
...
protected:
	...
	bool assertImplementation(bool condition, TString conditionExpression, long lineNumber, TString fileName, TString message);

	bool assertImplementation(bool condition, TString conditionExpression = "", long lineNumber = IT_TESTFW_UNKNOWNLINENUMBER,
							  TString fileName = IT_TESTFW_UNKNOWNFILENAME);

	bool assertEquals(long expected, long actual, long lineNumber, TString fileName, TString message);

	bool assertEquals(long expected, long actual, long lineNumber = IT_TESTFW_UNKNOWNLINENUMBER,
					  TString fileName = IT_TESTFW_UNKNOWNFILENAME);

	bool assertEquals(double expected, double actual, double delta, long lineNumber, TString fileName, TString message);

	bool assertEquals(double expected, double actual, double delta, long lineNumber = IT_TESTFW_UNKNOWNLINENUMBER,
					  TString fileName = IT_TESTFW_UNKNOWNFILENAME);

	bool assertEquals(const char *expected, const char *actual, long lineNumber = IT_TESTFW_UNKNOWNLINENUMBER,
					  TString fileName = IT_TESTFW_UNKNOWNFILENAME);

	bool assertEquals(const char *expected, long lengthExpected, const char *actual, long lengthActual,
					  long lineNumber = IT_TESTFW_UNKNOWNLINENUMBER, TString fileName = IT_TESTFW_UNKNOWNFILENAME);

	bool assertEquals(const char *expected, const char *actual, long lineNumber, TString fileName, TString message);

	bool assertEquals(const char *expected, long lengthExpected, const char *actual, long lengthActual, long lineNumber,
	...
	TString notEqualsMessage(const char *expected, long lengthExpected, const char *actual, long lengthActual) const;

	bool assertEqualsIfNullPtr(const char *expected, const char *actual, long lineNumber, TString fileName, TString message);

	template <typename T>
	bool assert_equal_to(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
	...
	template <typename T>
	bool assert_not_equal_to(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
		TString strExpression;
		strExpression.Append((unsigned long long)left)
			.Append(" not_equal_to ")
			.Append((unsigned long long)right)
			.Append(addexpr);
		return assertImplementation(not_equal_to<T>()(left, right), strExpression, lineNumber, fileName, message);
	}
	template <typename T>
	bool assert_less(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
		TString strExpression;
	...
	template <typename T>
	bool assert_less_equal(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
		TString strExpression;
	...
	template <typename T>
	bool assert_greater(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
		TString strExpression;
	...
	template <typename T>
	bool assert_greater_equal(T left, T right, TString addexpr, long lineNumber, TString fileName, TString message) {
		TString strExpression;
		strExpression.Append((unsigned long long)left)
...
#define t_assert(condition) \
	(this->assertImplementation(static_cast<bool>(condition), (#condition), static_cast<long>(__LINE__), __FILE__))
...
#define assertDoublesEqual(expected, actual, delta)                                           \
	(this->assertEquals((expected), (actual), (delta), static_cast<long>(__LINE__), __FILE__, \
...
#define t_assertm(condition, message) \
	(this->assertImplementation((bool)(condition), (#condition), static_cast<long>(__LINE__), __FILE__, message))

#define assertDoublesEqualm(expected, actual, delta, message) \
	(this->assertEquals((expected), (actual), (delta), static_cast<long>(__LINE__), __FILE__, message))

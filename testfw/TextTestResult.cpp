...
*/
{
	...
	TestLocation *cause = NULL;
	for (cause = causes.first(); cause != 0; cause = causes.next()) {
		stream << i << ") " << cause->getTestName() << ": " << cause->getFileName() << ":" << cause->getLineNumber() << ": "
			   << cause->getMessage() << std::endl;
		++i;
	}
}
...
void TextTestResult::printHeader(std::ostream &stream) {
	if (wasSuccessful())
		std::cout << "OK (" << runTests() << " tests and " << testSuccesses() << " assertions in " << elapsedTime() << " ms)"
				  << std::endl;
	else
		std::cout << std::endl
				  << "!!!FAILURES!!!" << std::endl
				  << "Test Results:" << std::endl
				  << "Run:  " << runTests() << "   Failures: " << testFailures() << "   Errors: " << testErrors() << std::endl
